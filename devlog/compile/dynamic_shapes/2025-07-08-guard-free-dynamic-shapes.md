...
> exporting models with dynamic shapes.  We launched an initiative to
> eliminate them via **explicit unbacked semantics** — explicitly defining
> how code should behave when inputs are unbacked.  In the first half, we
> generalized 60%+ of the relevant code, achieving a **55% reduction** in
> framework DDEs and saving weeks of engineering time on complex model
> exports.
...
contiguous inputs.  In those situations we want tracing modes that avoid
guards on dynamic shapes whenever possible by taking guard-free general
paths.
...
**all** input shapes.  To make this approach sound, we need to ensure that
dynamic shapes in the graph aren't constrained by guards — using
guard-free dynamic shapes.
...
- Address vLLM dynamic shapes soundness issues by tracing with guard-free
  dynamic shapes.
