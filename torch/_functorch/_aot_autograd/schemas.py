...
class ViewAndMutationMeta:
    ...
    # Number of DeviceMesh get_attr nodes hoisted to placeholders in the joint
    # graph. These become extra primals whose values are derived at runtime from
    # the parent DeviceMesh already present among the function args (via Dynamo
    # hoisting + DTensor flatten). The wrapper uses hoisted_device_mesh_info to
    # know which submeshes to derive.
    num_hoisted_device_meshes: int = 0
    hoisted_device_mesh_info: list[dict[str, Any]] = field(default_factory=list)
